...
if($pass_correctas){ // SI HAY QUE TENER EN CUENTA LA CONTRASEÑA Y SON CORRECTAS
	...
	if($nombre != "" && $cif != "" && $direccion != "" && $poblacion != "" && $cp != "" && $provincia != "" && $email != "" && $telefono != ""){
		...
		$ok = $db->update('usuarios_mod',$set,'id=(SELECT id_usuarios_mod FROM usuarios WHERE id = '.$id.')');
		
		if($ok){ // SI GUARDA LOS DATOS
		
			$ok = $db->update('usuarios','tarifa='.$tarifa.', descuento='.$descuento, 'id='.$id);
			...
			if($ok){ // SI TODO SE HA GUARDADO BIEN
				...
				$respuesta['estado'] = 'ko';
				$respuesta['mensaje'] = 'Error al guardar la tarifa de usuario. Inténtelo de nuevo.';
			...
			// SE PORODUJO UN ERROR AL GUARDAR LOS DATOS
			$respuesta['estado'] = 'ko';
			$respuesta['mensaje'] = 'Error al guardar los datos de usuario. Inténtelo de nuevo.';
		...
		$respuesta['estado'] = 'ko';
		$respuesta['mensaje'] = 'Error al guardar los datos de usuario. Revise los campos obligatorios.';
	...
	$respuesta['estado'] = 'ko';
	$respuesta['mensaje'] = 'Error al guardar los datos de usuario. Las contraseñas no son iguales.';
