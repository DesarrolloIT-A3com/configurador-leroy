	...
	<body>
		<div style="float: left; width: 100%; margin: 10mm;">
			...
			<table style="margin-top: 10mm;">
				<tr>
					<td style="width: 33.33%; box-sizing: border-box; padding-right: 10px; vertical-align: top;">
						<h2>Serie</h2>';
						if($serie){ $html .= $serie; }else{ $html .= "Solo interior"; } $html .= '
					</td>
					<td style="width: 33.33%; box-sizing: border-box; padding: 0px 10px; vertical-align: top;">
						<h2>Acabado</h2>';
						if($acabado){ $html .= $acabado; }else{ $html .= "-"; } $html .= '
					</td>
					<td style="width: 33.33%; box-sizing: border-box; padding-left: 10px; vertical-align: top;">
						<h2>Color perfilería</h2>';
						if($perfileria){ $html .= $perfileria['nombre'].'<br /><img src="../www/img/colores/'.$perfileria['imagen'].'" title="'.$perfileria['nombre'].'" width="100px" />'; }else{ $html .= "-"; } $html .= '
					</td>
				</tr>
				<tr>
					<td style="width: 33.33%; box-sizing: border-box; padding-right: 10px; vertical-align: top;">
						...
						<table style="margin-top: 10mm;" cellspacing="0">
							...
							<tr>';
								...
								<td class="item_fila_proyecto_datos_interior" style="width: 25%; padding: 12px 1%; vertical-align:top;">
									...
									Doble<br />';
									if(${'interior_puerta_'.$num_modulo}[2] > 0){ $html.= 'Zapatero/s con freno<br />'; }
									if(${'interior_puerta_'.$num_modulo}[3] > 0){ $html.= 'Cajon/es con freno<br />'; }
									if(${'interior_puerta_'.$num_modulo}[4] > 0){ $html.= ${'interior_puerta_'.$num_modulo}[4].' x J. celdillas<br />'; }
									if(${'interior_puerta_'.$num_modulo}[5] > 0){ $html.= ${'interior_puerta_'.$num_modulo}[5].' x Frente cristal<br />'; }
									if(${'interior_puerta_'.$num_modulo}[6] > 0){ $html.= ${'interior_puerta_'.$num_modulo}[6].' x Cerradura<br />'; }
			...
			<table style="position:absolute; bottom: 0;">
				...
				$html .= str_pad($proyecto['id_tarifa'], 2, "0", STR_PAD_LEFT)."-".str_pad(number_format($precio_distribuidor_iva,2,".",""), 10, "0", STR_PAD_BOTH).' <br /> ';
				...
				$html .=  str_pad("2", 2, "0", STR_PAD_LEFT)."-".str_pad(number_format($precio_distribuidor_2 + $precio_distribuidor_2 * $proyecto['porcentaje_iva'] / 100,2,".",""), 10, "0", STR_PAD_BOTH).' 
					</td>
				</tr>
			</table>';
