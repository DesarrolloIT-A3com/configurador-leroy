...
$proyectos = $db->getResults('SELECT id, nombre_cliente, dni_cliente, poblacion_cliente, provincia_cliente, fecha_proyecto, enviado, fecha_enviado, eliminado, fecha_eliminado FROM proyectos WHERE id_usuario='.$id.' '.$where.' ORDER BY '.$orderby.' '.$order);

//Consultamos cuantos hay de cada tipo
$consulta = "SELECT COUNT(*) FROM proyectos WHERE id_usuario=".$id;
$todos = $db->getVar($consulta);
$consulta = "SELECT COUNT(*) FROM proyectos WHERE id_usuario=".$id." AND enviado=0";
$no_enviados = $db->getVar($consulta);
$consulta = "SELECT COUNT(*) FROM proyectos WHERE id_usuario=".$id." AND enviado=1";
$enviados = $db->getVar($consulta);

?>
<div class="filtro_mostrar">
	<a class="<?= ($mostrar == 'todos') ? 'activo':''; ?>" onClick="listado_proyectos(<?php echo $id; ?>, 'todos')">Todos <span class="contador">(<span id="todos"><?=$todos?></span>)</span></a> | <a class="<?= ($mostrar == 'enviados') ? 'activo':''; ?>" onClick="listado_proyectos(<?php echo $id; ?>, 'enviados')">Enviados <span class="contador">(<span id="enviados"><?=$enviados?></span>)</span></a> | <a class="<?= ($mostrar == 'no_enviados') ? 'activo':''; ?>" onClick="listado_proyectos(<?php echo $id; ?>, 'no_enviados')">No enviados <span class="contador">(<span id="no_enviados"><?=$no_enviados?></span>)</span></a>
</div>
<div class="lista_proyectos">
		...
		<tbody>
			...
			if($proyectos){
				...
				<tr id="proyecto-<?php echo $id; ?>">
					<td><a class="ver_proyecto" title="Ver proyecto" href="index.php?seccion=usuarios&sub=ver_proyecto&id=<?php echo $id; ?>">P<?php echo str_pad($id, 6, "0", STR_PAD_LEFT); ?></a></td>
					...
					<td class="centrado"><?php if($proyecto['enviado'] == 1){ ?><a class="acciones editar" onClick="confirmar_editar_proyecto(<?php echo $id; ?>, 1);" title="Editar proyecto"><i class="fa fa-edit"></i></a><?php } ?>
					<a class="acciones eliminar" onClick="confirmar_borrar_proyecto(<?php echo $id; ?>);" title="Eliminar proyecto"><i class="fa fa-trash"></i></a></td>
				</tr>
				<?php
				}
			}
			...
			?>
				<tr><td colspan="8"><center>No hay proyectos</center></td></tr>
			<?php
			}
			?>
		</tbody>
	...
	<script type="text/javascript">
		$("#tabla_proyectos").tablesorter( {  // PARA PODER ORDENAR LA TABLA CON JQUERY
			...
			headers: { 4: { sorter: "shortDate" }, 7: { sorter: false } },   // EN TODAS LAS COLUMNAS
